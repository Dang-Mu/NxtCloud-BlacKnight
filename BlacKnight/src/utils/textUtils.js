...
export const highlightChanges = (oldText, newText) => {
  ...
  while (i < Math.min(oldWords.length, maxWords) && j < Math.min(newWords.length, maxWords)) {
    if (oldWords[i] === newWords[j]) {
      ...
      for (let k = 1; k <= 3 && i + k < oldWords.length; k++) {
        if (oldWords[i + k] === newWords[j]) {
          // 삭제된 단어들 (빨간색 취소선)
          for (let l = 0; l < k; l++) {
            result += `<span style="color: red; text-decoration: line-through">${oldWords[i + l]}</span> `;
          }
          i += k;
          found = true;
          break;
      ...
      if (!found) {
        ...
        for (let k = 1; k <= 3 && j + k < newWords.length; k++) {
          if (newWords[j + k] === oldWords[i]) {
            // 추가된 단어들 (녹색)
            for (let l = 0; l < k; l++) {
              result += `<span style="color: green; font-weight: bold">${newWords[j + l]}</span> `;
            }
            j += k;
            found = true;
            break;
      ...
      if (!found) {
        // 변경된 단어 (노란색 배경)
        result += `<span style="background-color: yellow">${newWords[j]}</span> `;
        i++;
        j++;
      }
    }
