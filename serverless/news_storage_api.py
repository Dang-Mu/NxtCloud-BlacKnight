...
def lambda_handler(event, context):
    ...
    try:
        ...
        elif http_method == 'GET' and resource_path == '/articles/user/{userId}':
            # 사용자별 기사 목록 조회
            user_id = path_parameters.get('userId', '')
            return get_user_articles(user_info, user_id, headers)
        elif http_method == 'GET' and resource_path == '/articles/{articleId}/versions':
            # 기사 버전 목록 조회
            article_id = path_parameters.get('articleId', '')
            return get_article_versions(user_info, article_id, headers)
...
def save_article(user_info, article_data, headers):
    ...
    """
    # 필수 필드 검증
    required_fields = ['newsId', 'articleId', 'content', 'ownerId', 'version', 'description']
...
def get_user_articles(user_info, user_id, headers):
    ...
    logger.info(f"[ARTICLE] 사용자 기사 목록 조회 시작: userId={user_id}")
    ...
    try:
        ...
        logger.info(f"[ARTICLE] 사용자 기사 목록 조회 성공: userId={user_id}, 개수={len(articles)}")
...
def get_article_versions(user_info, article_id, headers):
    """
    특정 기사의 모든 버전 목록 조회
    """
    if not article_id:
    ...
    logger.info(f"[ARTICLE] 기사 버전 목록 조회 시작: articleId={article_id}")
    ...
    try:
        ...
        # 결과가 없는 경우
        if not versions:
            logger.warning(f"[ARTICLE] 기사 버전 없음: articleId={article_id}")
            return {
                'statusCode': 404,
                'headers': headers,
        ...
        # 권한 검증
        owner_id = versions[0].get('ownerId', '')
        requester_id = user_info.get('id', '')
        role = user_info.get('role', '')
        organization = user_info.get('organization', '')
        ...
        logger.info(f"[ARTICLE] 기사 버전 목록 조회 성공: articleId={article_id}, 개수={len(versions)}")
        
        return {
            'statusCode': 200,
            'headers': headers,
            'body': json.dumps(versions, default=str)
        }
    except Exception as e:
